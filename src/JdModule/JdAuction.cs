...
namespace JdModule
{
    public class JdAuction
    {
        ...
        int g_AdvanceSecond = 0;
        bool g_Debug = false;

        public bool BidDebug
        {
            get { return g_Debug; }
            set { g_Debug = value; }
        }
        ...
        public JdAuction(CookieCollection cookie, int advanceSec, bool debug)
        {
            g_Cookie = cookie;
            g_AdvanceSecond = advanceSec * 1000;
            g_Debug = debug;
        }
        ...
        public AuctionInfo GetAuctionInfo(string id)
        {
            ...
            info.Name = Regex.Match(response_txt, JdAuctionRegex.Name).Value;
            info.Used = Regex.Match(response_txt, JdAuctionRegex.Used).Value;

            double _time;
            if (double.TryParse(Regex.Match(response_txt, JdAuctionRegex.UnixTime).Value, out _time))
                info.UnixTime = _time;

            decimal _jdprice;
            if (decimal.TryParse(Regex.Match(response_txt, JdAuctionRegex.JdPrice).Value, out _jdprice))
        ...
        public string Bid(string id, decimal? nowPrice, bool debug = false)
        {
            ...
            if (nowPrice.HasValue)
            {
                ...
                code = Regex.Match(response_txt, JdAuctionRegex.Code, RegexOptions.IgnoreCase).Value;
            }
        ...
        public void AutoAuction(object auctionTask)
        {
            ...
            int sleeptime = (int)_betweentime.TotalMilliseconds - g_AdvanceSecond;
            ...
            else
            {
                ...
                if (lastprice.HasValue)
                {
                    ...
                    else
                    {
                        ...
                        string code = Bid(id, lastprice + bidprice, g_Debug);
                        msg = CreateBidMsg(id, code, bidprice);
                    }
                }
                else
                {
